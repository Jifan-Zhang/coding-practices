		...
		else if(nums1Size+nums2Size==4)
		{
			if(*nums1<*nums2){
				if(*(nums1+nums1Size-1)>*(nums2+nums2Size-1))
					return nums1Size==2?(float)(*nums2+*(nums2+1))/2:(float)(*nums1+*(nums2+1))/2;
				else
					return nums1Size==1?(float)(*nums2+*(nums2+1))/2:(float)(*(nums1+1)+*(nums2))/2;}
			else{
				if(*(nums1+nums1Size-1)>*(nums2+nums2Size-1))
					return nums1Size==2?(float)(*nums1+*(nums2+1))/2:(float)(*nums1+*(nums1+1))/2;
				else
					return nums1Size==1?(float)(*nums1+*(nums2+1))/2:(float)(*(nums1+1)+*(nums2))/2;}
		}
	}*/
	printf("数组1(%d)检测值：%d; 数组2(%d)检测值：%d\n",nums1Size,pos_1_val,nums2Size,pos_2_val);

	if(pos_1_val<pos_2_val){
		if(nums1Size<=target_count/2){
			printf("数字1清空\n");
			target_count-=nums1Size;
			return original_length%2?*nums2:(*nums2+*(nums2+1))/2;
		}
		...
		if(nums2Size<=target_count/2){
			printf("数字2清空\n");
			target_count-=nums2Size;
			return original_length%2?*nums1:(*nums1+*(nums1+1))/2;
		}
		nums2+=nums2Size/2;
